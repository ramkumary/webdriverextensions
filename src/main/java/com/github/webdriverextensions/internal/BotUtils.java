...
public class BotUtils {
	 ...
	 * @return
	 */
	public static WebElement findElementFirstOrDefault(ElementTag tagName, String attributeName,
			String attributeValue) {
		WebElement elementFound = null;
		String elementTag = tagName.toString();
		By elementBy = By.tagName(elementTag);
		List<WebElement> allWebElements = WebDriverExtensionsContext.getDriver().findElements(elementBy);
		if (!allWebElements.isEmpty()) {
			for (WebElement eachElement : allWebElements) {
				if (eachElement != null && eachElement.getAttribute(attributeName) != null
						&& eachElement.getAttribute(attributeName).equalsIgnoreCase(attributeValue)) {
					elementFound = eachElement;
					break;
				}
			}
		}
	...
	/**
	 * Search the web element using tag name and text associated with that and
	 * return the first found web element.
	 * 
	 * @param tagName
	 *            - Html tag name
	 * @param displayText
	 *            - Text associated with the html tag.
	 * @return First found web element
	 */
	public static WebElement findElementFirstOrDefault(ElementTag tagName, String displayText) {
		WebElement elementFound = null;
		List<WebElement> allWebElements = WebDriverExtensionsContext.getDriver().findElements(By.xpath("//*"));
		for (WebElement eachElement : allWebElements) {
			if (eachElement != null && eachElement.getTagName() != null
					&& eachElement.getTagName().equalsIgnoreCase(tagName.toString())
					&& eachElement.getText().equalsIgnoreCase(displayText)) {
				elementFound = eachElement;
				break;
			}
		}
